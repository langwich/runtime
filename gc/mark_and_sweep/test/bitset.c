...
// converts value to bitmask (value shifted to that position)
...
#endif
{
	/* Fast compute degree of a set... the number
	   ...
	   and that SETSIZE(a) is a multiple of WORDSIZE_IN_BITS.
	*/
	...
	while ( p < endp )
	{
		...
		do {
			if (t & *b) ++degree;
		} while (++b < &(bitmask[WORDSIZE_IN_BITS]));
		p++;
	}
...
unsigned int set_find_region( set a, unsigned int n )
{
	/* Find first n contiguous bits; return low bit number of region or -1.
	 * todo: doesn't work across words. only finds regions in words.
	 */
	register unsigned *p = &(a.setword[0]);
	register unsigned *endp = &(a.setword[a.n]);
	...
	// n = 3 => 0..0111. check at 0 position, if ~word & mask == mask we found it
	// else shift left a bit and try again (todo: can skip some)
	unsigned mask = ~(~0 << n); // 1..1000 then to 0..0111

	while ( p < endp ) {
		register unsigned word = *p;
		int i = 0; // index 0 (bit position)
		for (i=0; i<WORDSIZE_IN_BITS; i++) {
			// e.g., n=2, 1011001 word & 0..011 mask shifted left 1 fist region
			if ((~word & mask) == mask) {
				return i; // found region at low bit position i
			}
...
void set_orin_range( set *a, unsigned i, unsigned j )
{
	int x;
	for (x=i; x<=j; x++) {
		set_orel(x, a);
	}
	// TODO: slow. try masks below
//	unsigned wordnum = DIVWORD(i);
//	unsigned mask = ~(~0 << n); // n=3: 1..1000 then to 0..0111
//
//	unsigned *firstwd = &a.setword[DIVWORD(i)];
//	unsigned *lastwd = &a.setword[DIVWORD(j)];
//
//	unsigned word = a.setword[wordnum]; // get word holding low bits (or all)
//	mask <<= MODWORD(i); // shift into position
//	unsigned v = word | mask;
}
